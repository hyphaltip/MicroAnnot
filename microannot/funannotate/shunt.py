...
This module provides :class:`MicroAnnotShunt`, which:

1. Detects the ``--organism microsporidia`` flag in a funannotate predict run
   (or is invoked directly via the ``microannot-funannotate`` CLI).
2. Runs the full :class:`~microannot.pipeline.MicroAnnotPipeline` instead of
   the standard Augustus/GeneMark/SNAP pipeline.
3. Uses :mod:`microannot.funannotate.adapter` to convert MicroAnnot's output
...
plugin / wrapper script and call :meth:`MicroAnnotShunt.run`.  The resulting
...
from microannot.funannotate.adapter import convert_to_funannotate


class MicroAnnotShunt:
    ...
    def __init__(
        ...
        self._pipeline = MicroAnnotPipeline(
    ...
    def run(self) -> str:
        ...
        """
        # 1. Run the full MicroAnnot pipeline
        self._pipeline.run()
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="microannot-funannotate",
        description=(
            "Run the MicroAnnot microsporidia prediction shunt and produce\n"
    ...
    p.add_argument("--db-dir", "-d", required=True, metavar="DIR",
                   help="MicroAnnot database root directory.")
